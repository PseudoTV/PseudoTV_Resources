...
GITPATH    = 'Z:\GitHub\PseudoTV_Resources/'
...
class Generator(object):
    ...
    def create_zip_file( self, fpath, addon):
        ...
        zips = list(filter(lambda k:k.endswith('.zip'), (list(os.walk(os.path.join(ZIPPATH,addon)))[0][2])))
        for zip in zips:
            print ("removing old zip %s" % os.path.join(os.path.join(ZIPPATH, addon, zip)))
            os.remove(os.path.join(ZIPPATH, addon, zip))
